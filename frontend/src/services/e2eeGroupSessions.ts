...
function receivingKeyStorageKey(
  ...
  return `senderkey:recv:${chatId}:${senderUserId}:${senderDeviceId}:${chainId}`;
}
...
async function loadOwnState(chatId: string): Promise<SenderKeyState | null> {
  ...
  const stored = await idbGet<SerializedSenderKeyState>(ownKeyStorageKey(chatId));
  if (!stored) return null;
  const state = deserializeSenderKeyState(stored);
...
async function saveOwnState(chatId: string, state: SenderKeyState): Promise<void> {
  ownStateCache.set(chatId, state);
  await idbSet(ownKeyStorageKey(chatId), serializeSenderKeyState(state));
}
...
async function loadReceivingState(
  ...
  if (cached) return cached;
  const stored = await idbGet<SerializedReceivingChainState>(key);
  if (!stored) return null;
  const state = deserializeReceivingChainState(stored);
...
async function saveReceivingState(
  ...
  const key = receivingKeyStorageKey(chatId, senderUserId, senderDeviceId, chainId);
  receivingStateCache.set(key, state);
  await idbSet(key, serializeReceivingChainState(state));
}
 ...
 * so an offline target catches up later via fetchPendingDistributions.
 */
 ...
 * send, matching createGroup() costing nothing for an empty group (see
 * E2EE_PLAN.md). Distribution happens separately, on every send — see
 * encryptGroupMessage.
 */
 ...
 * chain key can't decrypt anything sent from now on (see senderKeys.ts's
 * rekey self-test). A no-op if this device never had an active chain for
 * this chat in the first place (never sent a group message here yet).
 */
 ...
 * joined — no-op if this device has no active chain yet (nothing to give
 * them) or if `newUserId` is this device's own user (nothing to send self).
 */
 ...
 * never got a real-time push for, e.g. it was mid-registration) — call on
 * socket reconnect. Every row is safe to reapply: applyDistributionMessage
 * no-ops if we already have that exact chain.
 */
 ...
 * hasn't seen a distribution for the sender's CURRENT chain yet — e.g. it
 * joined mid-conversation, or the real-time push raced with this message),
 * does one on-demand catch-up fetch and retries once before giving up.
 */
