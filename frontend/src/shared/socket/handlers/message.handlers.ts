...
const handleMessageSent = async ({ tempId, message }: { tempId: string; message: Message }) => {
  ...
  // server resolved no specific requester device for this generic DTO (see
  // messages.service.ts's create()), and the sender doesn't need to decrypt
  // their own message anyway. The optimistic temp entry already holds the
  // real plaintext (what the user just typed), so preserve it instead of
  // overwriting it with the ack's null content.
  ...
  // to your own device (you don't need to decrypt what you just typed), so
  // without caching it now, any future reload (which wipes the in-memory
  // store) would have no way to ever recover this message's content again.
  ...
  // descriptor in `content` — see ChatView.tsx's handleUploadAndSend) —
  // keep the optimistic entry's local blob-URL attachments so the sender's
  // own bubble doesn't flash into a "Decrypting…" state for a file this
  // device already has in memory.
  ...
  // Sync the lastMessage preview in the chat list. Unconditional (not
  // gated on an existing chat.lastMessage.id === tempId) because sending a
  // message never optimistically set chat.lastMessage in the first place —
  // this ack is the first and only point that does, for the sender's side.
  store.updateChat(message.chatId, {
    lastMessage: {
      id: message.id,
      content: resolvedContent ?? '',
      ...
      createdAt: message.createdAt,
      senderId: message.senderId,
      senderName: useAuthStore.getState().user?.profile?.displayName ?? null,
      status: 'sent',
    },
    updatedAt: message.createdAt,
  });
