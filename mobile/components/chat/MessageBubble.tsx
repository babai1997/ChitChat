...
export default function MessageBubble({
  message, isOwn, showSender, onEdit, onDelete,
}: MessageBubbleProps) {
  ...
  const Tick = () => {
    switch (message.status) {
      case 'sending':   return <Clock      size={11} color={TIME_COLOR} />;
      case 'sent':      return <Check      size={13} color={TIME_COLOR} />;
      case 'delivered': return <CheckCheck size={13} color={TIME_COLOR} />;
      case 'read':      return <CheckCheck size={13} color={READ_TICK} />;
      default:          return <Clock      size={11} color={TIME_COLOR} />;
    }
  };
  ...
  const Meta = ({ standalone = false }: { standalone?: boolean }) => (
    <View style={[styles.metaRow, standalone && styles.metaStandalone]}>
      ...
      {isOwn && <View style={{ marginLeft: 3 }}><Tick /></View>}
    </View>
  );
      ...
      <Wrapper isOwn={isOwn}>
        <View style={[
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center' }}>
            ...
            <Text style={{ fontSize: 14, fontStyle: 'italic', color: '#8696a0' }}>
      ...
      <Wrapper isOwn={isOwn}>
        <View style={[styles.bubble, isOwn ? styles.bubbleOwn : styles.bubbleTheirs]}>
          <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 4 }}>
            ...
            <Text style={[styles.msgText, { marginLeft: 8, color: isOwn ? OWN_TEXT : THEIR_TEXT }]}>
              {message.content}
            </Text>
          </View>
          <Meta standalone />
    ...
    <Wrapper isOwn={isOwn}>
      ...
      >
        ...
        {showSender && message.sender && (
          <Text style={styles.senderName}>{message.sender.displayName || 'Unknown'}</Text>
        )}
          ...
          <View style={{ marginBottom: hasText ? 4 : 0 }}>
            {message.attachments!.map((att) =>
              message.type === 'image' ? (
                ...
                <View key={att.id} style={styles.fileAttach}>
        ...
        {/* Message text */}
        {hasText && (
          <Text style={[styles.msgText, { color: isOwn ? OWN_TEXT : THEIR_TEXT }]}>
            {message.content}
          </Text>
        )}
          ...
          <View style={styles.menu}>
            {isOwn && message.type === 'text' && (
              <TouchableOpacity
                ...
                onPress={() => { setShowMenu(false); onEdit?.(message.id, message.content); }}
              >
            ...
            <TouchableOpacity
              style={styles.menuRow}
              onPress={() => { setShowMenu(false); onDelete?.(message.id, false); }}
            >
            ...
            {isOwn && (
              <TouchableOpacity
                style={styles.menuRow}
                onPress={() => { setShowMenu(false); onDelete?.(message.id, true); }}
              >
                <Trash2 size={18} color="#ef4444" />
                <Text style={[styles.menuTxt, { color: '#ef4444' }]}>Delete for everyone</Text>
              </TouchableOpacity>
            )}
          </View>
        </Pressable>
...
// ── Wrapper ─────────────────────────────────────────────────────────────────
function Wrapper({ isOwn, children }: { isOwn: boolean; children: React.ReactNode }) {
  return (
