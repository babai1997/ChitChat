...
// Phase 4a (device-linking history sync, see E2EE_PLAN.md) — mirrors
// frontend/src/services/deviceLinkSync.ts. A session between two devices
// of the SAME user is not a special case of Session — it's the exact same
// generic (userId, deviceId) pairwise primitive Phase 1 uses between
// different users.

const HISTORY_SYNCED_FLAG_KEY = 'chitchat_e2ee_history_synced_at';
 ...
 * succeeds — never depends on receiving any event back.
 */
export async function pushHistoryToDevice(newDeviceId: string): Promise<number> {
...
export async function approveDeviceLink(newDeviceId: string): Promise<number> {
  await devicesApi.approveDevice(newDeviceId);
  return pushHistoryToDevice(newDeviceId);
}
...
export async function fetchPendingHistoryPayloads(): Promise<void> {
  try {
    ...
    await AsyncStorage.setItem(HISTORY_SYNCED_FLAG_KEY, new Date().toISOString());
  } catch (err) {
...
export async function checkDeviceLinkStatus(): Promise<void> {
  try {
    const alreadySynced = await AsyncStorage.getItem(HISTORY_SYNCED_FLAG_KEY);
